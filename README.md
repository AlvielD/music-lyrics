...
> 🔗 Ontology URI [https://w3id.org/polifonia/ontology/music-lyrics/](ontology/music-lyrics.rdf)

![](ontology/music-lyrics_grafoo.png)


## Competency Questions
The structure of our ontology was developed based on the list of **competency questions** provided in the table below. These questions can be queried by running [this notebook](sparql/queries.ipynb).
...
You can look at the structure of a converted file [here](conversion/saved/00000000.json).
...
<br>We implemented english to be the standard, that required us to create a [translation dictionary](conversion/header_synonyms.py).
- No convention between different lyrics platforms for the spelling of interjections or for the adlibs:
<br>We had to use functions matching the longest substring (cf. in [utils.py](conversion/utils.py)) instead of absolute comparison ones.

## Data JSONification
Once that we had well planned and implemented everything, we were ready to carry out the whole conversion. You can follow the preparatory steps we did in the [test_conversion](conversion/test_conversion.ipynb) notebook.<br>
The successful results obtained after this preliminary step then led us to use the bulk functions defined in [main.py](conversion/main.py) and then perform some post-conversion steps so as to obtain out final dataset folder. You can follow these steps in the [real_conversion](conversion/real_conversion.ipynb) notebook.
<br><br>
